...
`data-*` attributes on the `<script>` tag ΓÇö **no code edits needed** to set up
a new page. Just copy one of the snippets below into Webflow's page settings
ΓåÆ Custom Code ΓåÆ *before* `</body>`.
...
behavior) ΓÇö good for general pages that don't have a more specific angle.
...
| `custom` | One-off CTA you define inline | Anything else ΓÇö promos, campaigns, one-time landing pages |
...
For a page that needs its own link ΓÇö a promo, a campaign, anything not
covered by the presets above ΓÇö use `data-variant="custom"` with:

- `data-cta-name` ΓÇö the button/card label (required, defaults to "Learn More")
- `data-cta-href` ΓÇö the destination URL (required, defaults to "#")
...
Every variant ΓÇö including the default ΓÇö can have its headline and subhead
...
<script src="https://cdn.jsdelivr.net/gh/thadwarren-eb/eb-exit-popup/exit-intent-popup.js" defer
  ...
  data-subhead="See exactly where your money's going ΓÇö 30 seconds, free."></script>
```
...
- **Suppressed for 7 days** after being shown or dismissed, via the
  `eb_exit_popup_seen` cookie ΓÇö shared across all variants and pages, so one
  visitor won't get shown a different popup right after dismissing another.
- **`debug: true`** by default ΓÇö open the console and look for `[eb-exit]`
...
DevTools ΓåÆ Application ΓåÆ Cookies ΓåÆ delete `eb_exit_popup_seen` ΓåÆ reload.

## Files in this repo

- `exit-intent-popup.js` ΓÇö the whole thing. Edit `VARIANTS`/`ICONS` here to
  add a new named preset or change styling.
- `mascot-celebration.json` ΓÇö Lottie animation for the single-CTA variants.
...
Open `demo.html` in a browser ΓÇö it dynamically injects the script and
supports `?variant=xray`, `?variant=switch`, `?variant=usage`, or
`?variant=custom` in the URL (note: some browsers restrict `fetch()` and
query strings on `file://` pages ΓÇö testing via a local dev server avoids
that).
